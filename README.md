...
Use CMake to create the taglib makefile.

    cd ..\taglib-1.11.1
    cmake -DZLIB_INCLUDE_DIR=c:\projects\zlib-1.2.8\ -DZLIB_LIBRARY=c:\projects\zlib-1.2.8\Release\zlibstatic.lib -DCMAKE_BUILD_TYPE=Release -DWITH_ASF=ON -DWITH_MP4=ON -DBUILD_EXAMPLES=ON -DBUILD_SHARED_LIBS=ON .

Then build taglib:

    msbuild /P:Configuration=Release INSTALL.vcxproj

Finally, build tcltaglib. Enter tcltaglib\win folder (if user wants to build x86 version), using:

    nmake -f makefile.vc MACHINE=IX86
    nmake -f makefile.vc install

* NOTE: Please put tag_c.dll and tag_c.lib to win\taglib\lib to build tcltaglib.

And user needs to put tag_c.dll and tag.dll to c:\Windows (or your windows folder or avaiable folder).
